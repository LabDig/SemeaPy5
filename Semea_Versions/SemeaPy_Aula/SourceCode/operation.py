...
def SeedSpeed(pin):
    ...
    if (aux_i_seed==20):
        seed_speed_array=np.append(seed_speed_array,(60)/(time.time()-time_start_seed))
...
def WheelSpeed(pin):
    ...
    if (aux_i_wheel==20): 
        wheel_speed_array=np.append(wheel_speed_array,(2.0)/(time.time()-time_start_wheel))
...
def ControlSpeedSeed(pinEnable_Seed,pinPWM_Seed,rot_seed,real_rot_seed):
    global seed_cor
    dt=(1.4978*rot_seed+13.1860)#+seed_cor
    if dt>100.0 : dt=100.0
    if dt<25 : dt=25.0
    PWM.set_duty_cycle(pinPWM_Seed,dt)
    GPIO.output(pinEnable_Seed,GPIO.HIGH)
    seed_cor=seed_cor+(rot_seed-real_rot_seed)/2
    print (dt,seed_cor)
